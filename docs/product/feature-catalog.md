...
| GPU acceleration toggle (capability-gated) | allows supported devices to opt into faster decode path | backend capability detection + safe fallback behavior | Implemented | Implemented (toggle disabled when unsupported, persisted when supported) |
