...
2. Packaging/runtime optimization: optional OpenCL/Hexagon accelerator offload in packaged runtime when device/backend support exists.
...
1. Delivers local-first behavior now with production runtime path (`in_process` by default) and deterministic backend labeling.
...
| Android GPU path | OpenCL/Hexagon offload (capability-gated) | Implemented (device/backend dependent) |
