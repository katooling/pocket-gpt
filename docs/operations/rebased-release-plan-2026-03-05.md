...
4. Decision policy is fixed: `internalDownload first` + `soft gate`.
...
2. Execute `QA-WP13-RUN02` moderated 5-user packet.
3. Execute `MKT-08` proof asset capture + listing shotlist QA.

### Phase 3 (March 13-15): Promotion Decision

1. Execute `MKT-09` first 7-day channel scorecard.
2. Execute `QA-12` weekly matrix update.
3. Run `PROD-10` launch gate matrix decision memo.
...
4. No mandatory runtime API surface change is required for immediate launch unblock.

## Required Test Cases Before Next Promotion Step

1. `:apps:mobile-android:testDebugUnitTest` PASS.
2. `connectedStandardDebugAndroidTest` PASS on required-tier device.
3. `devctl lane maestro` PASS after preflight robustness patch.
4. `devctl lane journey` PASS with attached report artifacts.
